...
public OnPluginStart()
{
	...
	//Dynamic respawn mechanicssm_respawn_reinforce_time_subsequent = null,
...
void ResetSecurityLives()
{
	...
	if (g_isCheckpoint)
	{
		...
		if (g_iCvar_respawn_reset_type == 1 && g_iRespawn_lives_team_sec > 0)
			SecDynLivesPerPoint();
	}
	// Individual lives
		...
		if (IsPlayerAlive(medic) && (StrContains(g_client_last_classstring[medic], "medic") > -1))
		{
			...
			if (iNearestInjured != 0)
			{
				...
				if (iNearestInjuredNIP != 0)
				{
					...
					// Get distance string
					sDistance = GetDistanceString(fNearestDistanceNIP);
					// Get height string
...
String:GetDistanceString(Float:fDistance)
{
	...
	{
		// Meter
		Format(sResult, sizeof(sResult), "%.0f meters", fTempDistance);
	}
...
public AI_Director_SetDifficulty()
{
	...
	new 
		AID_ReinfAdj_med = 20, AID_ReinfAdj_high = 30, AID_ReinfAdj_pScale = 0,
...
{
	if(!IsPlayerAlive(client) && g_iHurtFatal[client] == 0)
	{
		g_iHurtFatal[client] = 1;
		// Remove network ragdoll associated with player
